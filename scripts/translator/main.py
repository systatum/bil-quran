...
from rating import RateJob, RateResult, RatingService
...
finished_tl_jobs: dict[uuid.UUID, TranslationResult] = {}
finished_tl_jobs_lock: threading.Lock = threading.Lock()
finished_rate_jobs: dict[uuid.UUID, RateResult] = {}
...
def get_tl_result(job_id: uuid.UUID) -> TranslationResult | None:
    collect_tl_results()
    with finished_tl_jobs_lock:
        return finished_tl_jobs.get(job_id)

def get_tl_result_blocking(job_id: uuid.UUID) -> TranslationResult:
...
def get_rate_result(job_id: uuid.UUID) -> RateResult | None:
    collect_rate_results()
    with finished_rate_jobs_lock:
        return finished_rate_jobs.get(job_id)

def get_rate_result_blocking(job_id: uuid.UUID) -> RateResult:
