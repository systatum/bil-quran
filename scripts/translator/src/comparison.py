...
def compare(model: ModelSetup, source: str, translation0: str, translation1: str) -> Comparison:
    ...
    return Comparison(probabilities=(float(probabilities[0][0]), float(probabilities[0][1])), choice=int(choice))


@dataclass(kw_only=True)
class CompareJob(Job[Comparison]):
    source: str
    translation0: str
    translation1: str
...
class ComparisonService(Service[Comparison]):
    ...
    def _run_job(self, job: Job[Comparison]) -> JobResult[Comparison]:
        ...
        try:
            result = compare(self._model, job.source, job.translation0, job.translation1)
