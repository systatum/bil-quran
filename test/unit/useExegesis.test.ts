...
import ALIQULI_ABOUT from "../../public/quran/exegesis/aliquli/about.json"
...
>
const mockCreate = repo.exegesis.create as jest.MockedFunction<
  typeof repo.exegesis.create
>
...
const ABOUT_ASSET = ALIQULI_ABOUT as ExegesisMetadata

function alreadyInDb() {
  mockFindAllBy.mockResolvedValue({
    succeed: true,
    data: [{ id: "aliquli/en-US" } as any],
  })
}
...
describe("useExegesis", () => {
  ...
  it("returns null content when the verse key is absent from the chapter data", async () => {
    ...
    await waitFor(
      () => !result.current[0].content || result.current[0].content === null,
    )
