...
import { openExegesisDialog } from "./tools/interactivity"
import { enableExegesis, visitFresh } from "./tools/state"

test.describe("ExegesisDialog", () => {
  test.beforeEach(async ({ page }) => {
    await visitFresh(page)
    await enableExegesis(page)
  })
  ...
  test("footnotes render for verse with footnote markers", async ({ page }) => {
    const dialog = await openExegesisDialog(page, "1:7")
    await expect(dialog).toBeVisible({ timeout: 8_000 })
  ...
  test("clicking a Q-marker changes the verse indicator", async ({ page }) => {
    const dialog = await openExegesisDialog(page, "1:7")
    await expect(dialog).toBeVisible({ timeout: 8_000 })
  ...
  test("prev button is disabled on first verse", async ({ page }) => {
    ...
    await expect(dialog.locator('[data-testid="prev-verse-btn"]')).toBeDisabled({ timeout: 5_000 })
  })

  test("next button is disabled on last verse of chapter", async ({ page }) => {
    const dialog = await openExegesisDialog(page, "1:7")
    await expect(dialog).toBeVisible({ timeout: 8_000 })
    ...
    await expect(dialog.locator('[data-testid="next-verse-btn"]')).toBeDisabled({ timeout: 5_000 })
  })
  ...
  test("interlinear section default to 30% height", async ({ page }) => {
    ...
    const ratio = await dialog.evaluate((dialogEl) => {
      ...
      if (!interlinear || !interlinear.querySelector(".arabic-lex")) return null
